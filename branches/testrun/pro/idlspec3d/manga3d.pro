...
;       Additional tweak in tags/v0_5 to fix issue with not enough
;       good values to combine near ends of wavelength range.
;   v1.5: 29-May-2013 D. Law
;       Modifications to improve RSS file creation
;   v1.6: 04-Jun-2013 D. Law
;       Added /edgesky flag to use edge fibers as skies.
;       WARNING- very restricted usage to sky plates 6613 & 6614
;       Many sections hard coded
;
...
pro manga3d, input=input, outname=outname, inspect=inspect, doall=doall, edgesky=edgesky
...
for i=0,nexp-1 do begin
  ...
  status=mlimportspec(wave,redx[i],obsparam,slitmap,bmap,spec,specivar,xinbundle,yinbundle,fstat,filepath,flavor,doall=doall,edgesky=edgesky)
...
splog,'Making coverage map',prelog=''
...
for k=0,nwave-1 do begin
  ...
  ; Require at least nftotal/2 good values in order to combine
  ; (only really an issue towards extreme ends of wavelength range)
  if (((size(goodloc))[0] ne 0)and((size(goodloc))[1] gt 0.5*nftotal)) then begin
    xgood=x[goodloc]
    ygood=y[goodloc]
    datgood=dat[goodloc]
...
; Make an RSS file
mkhdr,rsshead,dataarray
outfilerss=strcompress(outpath+outname+'_rss.fits',/remove_all)
   ; HDU #0 is spectra
mwrfits, dataarray, outfilerss, rsshead, /create
   ; HDU #1 is ivar
mwrfits, ivararray, outfilerss
   ; HDU #2 is wavelength
mwrfits, wave, outfilerss
   ; HDU #3 is x table
mwrfits, (xrel-Xsize/2.)*pixscale, outfilerss
   ; HDU #4 is y table
mwrfits, (yrel-Ysize/2.)*pixscale, outfilerss
