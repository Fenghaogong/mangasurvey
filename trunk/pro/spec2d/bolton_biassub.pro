...
function bolton_biassub, rawdata, biasname, sigthresh=sigthresh, rnoise=rnoise, cam=cam, notrim=notrim
...
; Get data and bias subimages, but only where they are non-zero (so as to handle sub-frame readout gracefully)
...
if (~keyword_set(notrim)) then data_img = data_img[dx0:nxfull-dx0-1,dy0:nyfull-dy0-1]
