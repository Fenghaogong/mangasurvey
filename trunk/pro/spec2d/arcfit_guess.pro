...
function tset_struc, func, ncoeff, ntrace
  ...
  compile_opt idl2, hidden
   ...
   if (~keyword_set(ntrace)) then ntrace = 1
...
function arcfit_iter, spec, loglam, intensity, $
 aset, dcoeff, nsteps, nsmooth=nsmooth, dlag=dlag, bestcorr=bestcorr
   ...
   npad = long((npix+1)/2) ; Padding on left and right of spectra before cross-correlating
   ...
   splog, 'Searching lags from ', min(lags), ' to ', max(lags), ' spaced ', dlag, ' pixels', format='(a,i5,a,i5,a,i3,a)'
      ...
      if (loglambda[1] GT loglambda[0]) then begin ; Ascending wavelengths
         for iline=0, nline-1 do begin
            ...
            if (iloc GE 1 AND iloc LE npix-2) then begin
               ...
               model[npad+iloc:npad+iloc+1] = model[npad+iloc:npad+iloc+1] + intensity[iline] * [1-dx/dpix, dx/dpix]
            endif
         endfor
   ...
   xy2traceset, dindgen(npix)-lagbest, bestlambda, wset, ncoeff=ncoeff, xmin=0, xmax=npix-1, maxiter=1
...
function arcfit_guess, spec, loglam, intensity, color=color, func=func, $
 bestcorr=bestcorr, acoeff=acoeff, dcoeff=dcoeff, nsteps=nsteps
   ...
   if ~(keyword_set(acoeff) and keyword_set(dcoeff)) then begin 
   ...
   wset = arcfit_iter(spec, loglam, intensity, aset, dcoeff, nsteps, nsmooth=6.0, dlag=2, bestcorr=bestcorr)

   ; Second search varies 4 coefficients, but narrows search window for the first two coefficients
   ...
   wset = arcfit_iter(spec, loglam, intensity, wset, dcoeff, nsteps, nsmooth=4.0, dlag=1, bestcorr=bestcorr)
