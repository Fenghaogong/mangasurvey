...
function readplugmap_sort, plugmap, fibermask=fibermask
  ...
  compile_opt idl2, hidden
   ...
   if (~keyword_set(fibermask)) then fibermask = bytarr(nfiber) $
    else if (n_elements(fibermask) NE nfiber) then message, 'Number of elements in FIBERMASK do not match NFIBER'
   ...
   if (ngood EQ 0) then message, 'No fibers found in plugmap!'
...
function readplugmap, plugfile, spectrographid, plugdir=plugdir, $
 ...
 hdr=hdr, fibermask=fibermask, _EXTRA=KeywordsForPhoto
  ...
  compile_opt idl2
   ...
   if (keyword_set(fibermask)) then message, 'FIBERMASK is already set!'
   ...
   thisfile = file_search(djs_filepath(plugfile, root_dir=plugdir), count=ct)
   ...
   if (~keyword_set(pstruct)) then begin
   ...
   plugmap = struct_addtags(plugmap, replicate(create_struct('OFFSETID', 0L, 'SCI_EXPTIME', 0.), n_elements(plugmap)))
   ...
   if (ct GT 0) then begin
      ...
      for j=0L, n_elements(plugpoint)-1L do begin
         k = where(abs(plugmap.xfocal - plugpoint[j].xfocal) LT 0.0001 AND abs(plugmap.yfocal - plugpoint[j].yfocal) LT 0.0001, ct)
         if (ct GT 0) then begin
   ...
   if (keyword_set(platelist_dir)) then begin
      thisfile = file_search(djs_filepath(platefile, root_dir=platelist_dir, subdir=['plates','*','*']), count=ct)
      if (ct GT 0) then begin
         ...
         for i=0L, n_elements(plugmap)-1 do isort[i] = where(plateholes.xfocal EQ plugmap[i].xfocal AND plateholes.yfocal EQ plugmap[i].yfocal)
         ...
         htags = ['SOURCETYPE','LAMBDA_EFF','ZOFFSET','BLUEFIBER', 'BOSS_TARGET*','ANCILLARY_TARGET*','RUN','RERUN','CAMCOL','FIELD','ID']
         plugmap = struct_addtags(plugmap, struct_selecttags(plateholes, select_tags=htags))
         ...
         washers_file = getenv2('MANGADRP_DIR')+getenv2('MANGADRP_VER')+'opfiles/washers.par'
         ...
         if (n_elements(ii) gt 1) then message, "ERROR: multiple washers.par entries for " + plugname
...
;----------------
   ...
   if (keyword_set(calibobj)) then begin
      ...
      addtags = replicate(create_struct('CALIBFLUX', fltarr(5), 'CALIBFLUX_IVAR', fltarr(5),'CALIB_STATUS', lonarr(5),'SFD_EBV', 0.), n_elements(plugmap))
      ...
      if (keyword_set(tsobj)) then begin
         ...
         splog, 'Matched ', fix(total(qsky EQ 0 AND qexist)), ' of ', fix(total(qsky EQ 0)), ' non-SKY objects'
      ...
      if (keyword_set(tsobj)) then begin
         ...
         if tag_exist(tsobj, 'CALIB_STATUS') then plugmap.calib_status = tsobj.calib_status
         ...
         if (ngal GT 0) then begin
            for ifilt=0, 4 do begin
               plugmap[igal].calibflux[ifilt] = fiberflux[igal,ifilt] * pratio[ifilt]
               plugmap[igal].calibflux_ivar[ifilt] = fiberflux_ivar[igal,ifilt] / (pratio[ifilt])^2
            endfor
         endif

         ; Reject any fluxes based upon suspect PHOTO measurements, as indicated by the PHOTO flags.
         badbits2 = sdss_flagval('OBJECT2','SATUR_CENTER') OR sdss_flagval('OBJECT2','INTERP_CENTER') OR sdss_flagval('OBJECT2','PSF_FLUX_INTERP')
      ...
      for ifilt=0, 4 do begin
         ibad = where(plugmap.calibflux[ifilt] EQ 0 AND plugmap.mag[ifilt] GT 0 AND plugmap.mag[ifilt] LT 50, nbad)
         if (nbad GT 0) then begin
            splog, 'Using plug-map fluxes for ', nbad, ' values in filter ', ifilt
            plugmap[ibad].calibflux[ifilt] = 10.^((22.5 - plugmap[ibad].mag[ifilt]) / 2.5)
      ...
      for j=0,4 do plugmap.calibflux_ivar[j] = plugmap.calibflux_ivar[j] / factor[j]^2
   endif
   
;------------------
   if (keyword_set(deredden)) then begin
      splog, 'Applying reddening vector ', redden_med
      for ifilt=0, 4 do plugmap.mag[ifilt] = plugmap.mag[ifilt] - redden_med[ifilt]
   endif
