...
pro extract_object, outname, objhdr, image, invvar, plugsort, wset, $
 ...
 bershady=bershady
  ...
  compile_opt idl2
   ...
   ml_expand, fiberparam, nfiber, totfiber, fsize=fsize 
   iskies = where(strtrim(plugsort.objtype,2) EQ 'SKY' AND plugsort.fiberid GT 0 AND (fibermask EQ 0) AND (fsize eq 2), nskies)
   ...
   splog, '5% and 95% count levels ', scrunch[scrunch_sort[i5]], scrunch[scrunch_sort[i95]]
...
;x   xyouts, 100, 0.08*!y.crange[0]+0.89*!y.crange[1], 'GREEN = Initial chisq extraction'
   ...
   skystruct = skysubtract(flux, fluxivar, plugsort, vacset, skysub, skysubivar, iskies=iskies, pixelmask=pixelmask, fibermask=fibermask, upper=3.0, $
                            lower=3.0, tai=tai_mid, nbkpt=nbkpt, survey=survey, fsize=fsize)
   if (~keyword_set(skystruct)) then return

   ;----------
   ; If any of the sky-fibers are bad, then re-do sky-subtraction.
   ibadfib = where(djs_median(skysub[*,iskies]^2 * skysubivar[*,iskies], 1) GT 2.0)               
   if (ibadfib[0] NE -1) then begin
      ...
      skystruct = skysubtract(flux, fluxivar, plugsort, vacset, skysub, skysubivar, iskies=iskies, pixelmask=pixelmask, fibermask=fibermask, upper=10.0, $
                              lower=10.0, tai=tai_mid, nbkpt=nbkpt, survey=survey, fsize=fsize)
      if (~keyword_set(skystruct)) then return
   endif
   ...
   if keyword_set(VISUAL) then getwindow,/open
   qaplot_skysub, flux, fluxivar, skysub, skysubivar, vacset, iskies, title=plottitle+objname+' 1D Sky-subtraction'
   ...
   if keyword_set(splitsky) then begin
      ; Split sky subtraction between halves of the CCD if requested (bolton@utah 2011).
      ...
      skystruct0 = skysubtract(flux[*,0:isplit], fluxivar[*,0:isplit], plugsort[0:isplit], vacset0, skysub0, skysubivar0, iskies=iskies0, pixelmask=pixelmask[*,0:isplit], $
                                fibermask=fibermask[0:isplit], upper=10.0, lower=10.0, tai=tai_mid, npoly=nskypoly, nbkpt=nbkpt, relchi2set=relchi2set0, newmask=newmask0,survey=survey, fsize=fsize)
      skystruct1 = skysubtract(flux[*,isplit+1:*], fluxivar[*,isplit+1:*], plugsort[isplit+1:*], vacset1, skysub1, skysubivar1, iskies=iskies1, pixelmask=pixelmask[*,isplit+1:*], $
                                fibermask=fibermask[isplit+1:*], upper=10.0, lower=10.0, tai=tai_mid, npoly=nskypoly, nbkpt=nbkpt, relchi2set=relchi2set1, newmask=newmask1,survey=survey, fsize=fsize)
      ...
      pixelmask = newmask
      ; These are needed for QA, although we'll only get the low-fiber-number picture in this case:
      skystruct = skystruct0
      relchi2set = relchi2set0
   endif else begin
      skystruct = skysubtract(flux, fluxivar, plugsort, vacset, skysub, skysubivar, iskies=iskies, pixelmask=pixelmask, fibermask=fibermask, upper=10.0, lower=10.0, tai=tai_mid, $
                              npoly=nskypoly, nbkpt=nbkpt, relchi2set=relchi2set, newmask=newmask,survey=survey,fsize=fsize)
       pixelmask = newmask
   endelse
   ...
   if keyword_set(VISUAL) then getwindow,/open
   qaplot_skysub, flux, fluxivar, skysub, skysubivar, vacset, iskies, title=plottitle+objname+' 2D Sky-subtraction'

   ;----------
   ; QA for 2 skylines in the blue (specify vacuum wavelengths below)
   if (color EQ 'blue') then begin
      if keyword_set(VISUAL) then getwindow,/open
      qaplot_skyline, 4359.5, flux, fluxivar, skysub, skysubivar, plugsort, vacset, iskies, fibermask=fibermask, dwave=4.0, tai=tai_mid, title=plottitle+objname+' Skyline Flux at 4359.5'
      qaplot_skyline, 5578.9, flux, fluxivar, skysub, skysubivar, plugsort, vacset, iskies, fibermask=fibermask, dwave=5.0, tai=tai_mid, title=plottitle+objname+' Skyline Flux at 5578.9'
   endif
   ...
   if (color EQ 'red') then begin
      if keyword_set(VISUAL) then getwindow,/open
      qaplot_skyline, 7343.0, flux, fluxivar, skysub, skysubivar, plugsort, vacset, iskies, fibermask=fibermask, dwave=7.0, tai=tai_mid, title=plottitle+objname+' Skyline Flux at 7343,0'
      qaplot_skyline, 8888.3, flux, fluxivar, skysub, skysubivar, plugsort, vacset, iskies, fibermask=fibermask, dwave=7.0, tai=tai_mid, title=plottitle+objname+' Skyline Flux at 8888.3'
   endif
   ...
   if (keyword_set(do_telluric) AND color EQ 'red')  then begin
      telluricfactor = telluric_corr(flambda, flambdaivar, vacset, plugsort, fibermask=fibermask, plottitle=plottitle+'Telluric correction for '+objname)
   ...
   finalmask = pixelmask
   for itrace=0, ntrace-1 do finalmask[*,itrace] = finalmask[*,itrace] OR fibermask[itrace]
   ...
   for j=0, n_elements(discards)-1 do finalmask = finalmask - q_nodata * (finalmask AND sdss_flagval('SPPIXMASK',discards[j]))

   ;----------
   ; Get an estimate of the relative chi^2 at each pixel. Do this with a simple linear interpolation.
   if (keyword_set(relchi2set)) then begin
      xx = 0
      traceset2xy, vacset, xx, loglam
      rchi2img = bspline_valu(loglam, relchi2set)
      ; Compute the mean relative chi2 of sky-subtraction, after masking bad regions of the CCD
      ...
      if (ct EQ 0) then skychi2 = 0. else skychi2 = mean(rchi2img[indx])
   endif else begin
