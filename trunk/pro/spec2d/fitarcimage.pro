...
pro fitarcimage, arc, arcivar, xcen, ycen, wset, wfirst=wfirst, $
 ...
 visual=visual, survey=survey, nbundle=nbundle, nfiber=nfiber, radius=radius1
   ...
   if (~keyword_set(aset)) then begin
      if (color NE 'blue' AND color NE 'red') then message, "SIDE must be set to 'blue' or 'red' if ANS not specified"
   endif
   if (~keyword_set(func)) then func = 'legendre'
   if (~keyword_set(ans)) then ans = 0
   if ~keyword_set(maxdev) then maxdev = 2.0d-5
   minlamp = 6 ; Minimum number of lamp lines

   t_begin = systime(1)

   ndim = size(arc, /n_dim)
   if (ndim NE 2) then  message, 'Expecting 2-D arc image'
   dims = size(arc, /dim)
   npix = dims[0]
   totfiber = dims[1]
   if (total(dims NE size(arcivar, /dim))) then message, 'ARC and ARCIVAR must have same dimensions'
   if (~keyword_set(fibermask)) then fibermask = bytarr(totfiber)
   if (~arg_present(row)) then row = totfiber/2
   if (~keyword_set(nmed)) then nmed = 5
   if (~keyword_set(thresh)) then begin
      if (color EQ 'blue') then thresh = 200
      if (color EQ 'red') then thresh = 500
   endif
   if (~keyword_set(ncoeff)) then ncoeff = 5
   ...
   if (keyword_set(lampfile)) then begin
      lampfilename = (file_search(getenv2('MANGADRP_DIR')+getenv2('MANGADRP_VER'), lampfile, count=ct))[0]
      if (ct EQ 0) then message, 'No LAMPFILE found '+lampfile
   endif else begin
      lampfilename = filepath('lamphgcdne.dat', root_dir=getenv2('MANGADRP_DIR')+getenv2('MANGADRP_VER'), subdirectory='etc')
      if (~keyword_set(lampfilename)) then message, 'No LAMPFILE found '+lampdefault
   endelse
   ...
   if (~keyword_set(aset)) then begin
      ...
      if (ngfiber EQ 0) then message, 'No unmasked fibers according to FIBERMASK'

      ;takes median of central nmed rows in arc image
      ...
      aset = arcfit_guess( spec, lamps.loglam, lamps.intensity, color=color, bestcorr=bestcorr, _EXTRA=KeywordsForArcfit_guess)
   ...
   ; Trim lamp list to only those within the wavelength range and denoted as good in the LAMPS structure.
   xstart = traceset2pix(aset, lamps.loglam)
   qtrim = xstart GT 0 AND xstart LT npix-2 AND lamps.good
   if (keyword_set(wrange)) then qtrim = qtrim AND lamps.loglam GT alog10(wrange[0]) AND lamps.loglam LT alog10(wrange[1])
   itrim = where(qtrim, ct)
   if (ct EQ 0) then message, 'No arc lines in wavelength range'
   xstart = xstart[itrim]
   lamps = lamps[itrim]     
   
   ;VISUAL
   if keyword_set(VISUAL) then begin
    getwindow,/open,next=wavesol
    ;h=bytscl(arc,min=0,max=255)
    ;h_eq_ct,h
    h=arc
    sub=row+[-15,+15]
    ;plot, h[*,360:390], yra=[360,390], ystyle=1,/nodata, charsize=2, xtit='X pix', ytit='Fiber Number', tit='Initial Lamp Wavelength Guess, '+strtrim(n_elements(lamps),2)+' lamps'
    ml_tvimage,h[*,sub[0]:sub[1]], /axis,yra=[sub[0],sub[1]],axkeywords={charsize:2,xtitle:'X pix',ytitle:'Fiber Number', title:'Initial Lamp Wavelength Guess, '+strtrim(n_elements(lamps),2)+' lamps'}
    oplot, xstart, fltarr(n_elements(lamps))+row, psym=2, color=!red
   endif
   ...
   ; Allow for a shift of up to 2 pixels in the initial centers, but only 0.5 pixels while tracing
   nlamp = n_elements(lamps)
   splog, 'Tracing', nlamp, ' arc lines'
   xcen = ml_trace_crude(arc, arcivar, yset=ycen, nave=1, nmed=1, xstart=xstart, ystart=row, maxshifte=0.5d, maxshift0=2.0d, nfiber=nfiber, radius=radius1)

  ;VISUAL
  if keyword_set(VISUAL) then begin
    yind=[totfiber-51,totfiber-1] 
    getwindow,/open
    !p.multi=[0,1,2]
    plot, h[2200:2600,yind[0]:yind[1]], xra=[2200,2600],yra=[yind[0]+1,yind[1]+1], ystyle=1,/nodata, charsize=2, xtit='X pix', ytit='Fiber Number', tit='Trace the rest',xtickformat='(A1)', ytickformat='(A1)'
    ml_tvimage,h[2200:2600,yind[0]:yind[1]], /axis, yra=[yind[0]+1,yind[1]+1], xra=[2200,2600],/overplot
    oplot, xcen[findgen(31)*25,*], ycen[findgen(31)*25,*], psym=2, color=!red
    p1=!p & xc1=!x & yc1=!y
    
    plot, h[2200:2600,sub[0]:sub[1]], xra=[2200,2600],yra=[sub[0],sub[1]], ystyle=1,/nodata, charsize=2, xtit='X pix', ytit='Fiber Number', tit='Trace the rest',xtickformat='(A1)', ytickformat='(A1)'
    ml_tvimage,h[2200:2600,sub[0]:sub[1]], /axis, yra=[sub[0],sub[1]], xra=[2200,2600],/overplot
    oplot, xcen[findgen(31)*25,*], ycen[findgen(31)*25,*], psym=2, color=!red
    p2=!p & xc2=!x & yc2=!y
    !p.multi=0
  endif

   ; Now fit traceset to trace_crude, this will "interpolate" or bad fibers as well as extend trace off CCD if need be.
   ...
   x1 = ml_trace_fweight(arc, xcrudefit, ycen, radius=radius, nfiber=nfiber)
   x1 = ml_trace_fweight(arc, x1, ycen, radius=radius, nfiber=nfiber)
   xmid = ml_trace_fweight(arc, x1, ycen, radius=radius, nfiber=nfiber)
   ...
   if (keyword_set(gauss)) then begin
     ...
     x2  = ml_trace_fweight(arc, xmid, ycen, invvar=arcivar, xerr=xerr, radius=radius, nfiber=nfiber)
     x2  = ml_trace_fweight(arc, x2, ycen, invvar=arcivar, xerr=xerr, radius=radius, nfiber=nfiber)
     xcen = ml_trace_fweight(arc, x2, ycen, invvar=arcivar, xerr=xerr, radius=radius, nfiber=nfiber)
   endelse
   ...
   for i=0, nmatch-1 do begin
      ...
      if (fracbad GT 0.10) then begin
         ...
         splog, 'Discarding line', i, ' at ', lamps[i].lambda, ' Ang: FRACBAD=', fracbad
      endif

      djs_iterstat, xdiff[*,i], sigma=sigma
      if (sigma GT 0.2) then begin
         rejline[i] = 'Reject-RMS'
         splog, 'Discarding line', i, ', at ', lamps[i].lambda, ' Ang: RMS=', sigma, ' pix'
      endif
   endfor
   ...
   if (ngood LT minlamp) then begin
      print, 'FLAG: ARC IMAGE HAS TOO FEW LINES'
   ...
   if (nlamp EQ 1) then ytmp = transpose(lamps.loglam * (dblarr(totfiber)+1)) else ytmp = lamps.loglam # (dblarr(totfiber)+1)
...
; Include 2-phase readout arguments if necessary:
   ...
   while (iiter LT nlamp) do begin
      ...
      xy2traceset, transpose(double(xcen)), ytmp, wset, func=func, ncoeff=ncoeff, maxiter=nlamp, maxrej=nlamp, maxdev=maxdev, $
                   xmin=0, xmax=npix-1, inmask=transpose(inmask)*rebin(rejline EQ '',nlamp,totfiber), $
                   outmask=outmask, yfit=yfit, xjumplo=xjumplo, xjumphi=xjumphi, xjumpval=xjumpval
      ydiff = ytmp - yfit

      if (total(outmask) EQ 0) then begin
         print, 'FLAG: NO GOOD VALUES IN OUTMASK in iteration', iiter
         flag = 'NO GOOD VALUES IN OUTMASK'
      ...
      for i=0, nlamp-1 do begin
         badper[i]=-1
         ;bundle loop ; to account for varying fibers per bundle
         for j=0,nbundle-1 do begin
               indx = where(outmask[i,*], ct)
               numstart = (j eq 0) ? 0 :  ((total(nfiber,/cumul))[j-1])
               numend = (total(nfiber,/cumul))[j]-1
               
               if (ct GT 2) then begin
                  djs_iterstat, ydiff[i,indx], median=med1, sigma=sig1
                  res1 = polyfitw(findgen(totfiber), ydiff[i,*], outmask[i,*], 2, yfit1)
                  offsets[i] = sig1 > abs(med1) > abs(max(yfit1))
                  ; For each lamp, compute max # of bad fibers per bundle
                  temp = total(reform(1L-outmask[i,numstart:numend],nfiber[j]))
                  badper[i]=(temp gt badper[i]) ? temp : badper[i]                 
               endif else begin
                  yfit1 = 0
      ...
      ; Preferentially mask any lines with any bundles of more than 5 bad, then after that mask any lines with bad deviations
      thismax = max(offsets*(rejline EQ '') + ((badper-5)>0)*(rejline EQ ''), iworst)
      if (thismax GT maxdev) then begin
         splog, 'Rejecting line at ', 10d0^lamps[iworst].loglam, ' Ang ' + rejline[iworst]
   ...
   if (keyword_set(csmooth)) then begin
      ...
      for i=0, n_elements(csmooth)-1 do begin
         poly_iter, lindgen(totfiber), wset_tmp.coeff[csmooth[i],*], 3, 3.0, ycoeff1
         wset_tmp.coeff[csmooth[i],*] = ycoeff1
      endfor
      wset_tmp2 = wset_tmp
      wset_tmp2.coeff[csmooth,*] = 0
      ; Compute the offsets from only the (smoothed) higher-order terms
      xoffset = traceset2pix(wset_tmp, alog10(lamps.lambda)) - traceset2pix(wset_tmp2, alog10(lamps.lambda))
      ; Re-fit, subtracting off those higher-order terms first
      xy2traceset, transpose(double(xncen)) - xoffset, ytmp, wset_lo, func=func, ncoeff=3, maxiter=0, xmin=0, xmax=npix-1, inmask=outmask, yfit=yfit
      wset.coeff[3:ncoeff-1,*] = wset_tmp.coeff[3:ncoeff-1,*]
      wset.coeff[0:2,*] = wset_lo.coeff[0:2,*]
   endif

   for i=0, nlamp-1 do splog, 'Line ', i, lamps[i].lambda, offsets[i], ' '+rejline[i]

   ; Replace all centroids in any bad fibers
   wset = fitarc_maskbadfib(wset, outmask)

   ;----------
   ; Look for large gaps before the first arc line, between any consecutive arc lines, are after the last arc line.

   ; Get the starting and ending wavelengths on the image, but LOGWMIN
   ; and LOGWMAX will be switched if wavelengths are descending; that's why I sort that list.
   ...
   for i=0, N_elements(logwdiff)-1 do begin
      if (logwdiff[i] GT 0.10) then begin
         ...
         splog, 'WARNING: Reject arc. Big wavelength gap from ', 10^logwlist[i], ' to ', 10^logwlist[i+1], ' Ang'
         wset = 0
         return
      endif else if (logwdiff[i] GT 0.05) then begin
         ; Warning for gaps as large as [3650,4095] or [8000,8976] Angstroms.
         splog, 'WARNING: Big wavelength gap from ', 10^logwlist[i], ' to ', 10^logwlist[i+1], ' Ang'
      endif
   endfor
   ...
   splog, 'Time ',systime(1)-t_begin, ' seconds elapsed', format='(A,F6.0,A)'

   lampfile = lampfilename ; Return the name of the lampfile used.
   lambda = lamps.lambda

   ; Replace the "measured" arc line positions with the fit positions
   ; Do this so that the sky-line fitting will use those fit positions for the arc lines
   xcen = tset_pix

  ;VISUAL
  if keyword_set(VISUAL) then begin
      !p=p1 & !x=xc1 & !y=yc1
      oplot, xcen[findgen(31)*25,*], ycen[findgen(31)*25,*], psym=4, color=!blue
      !p=p2 & !x=xc2 & !y=yc2
      oplot, xcen[findgen(31)*25,*], ycen[findgen(31)*25,*], psym=4, color=!blue
      !p.multi=0
  endif
