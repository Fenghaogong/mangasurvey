...
function fitdispersion, arc_flux, arc_fluxivar, xcen_inp, $
 sigma=sigma, ncoeff=ncoeff, xmin=xmin, xmax=xmax, medwidth=medwidth, $
  numBundles = numBundles, quick=quick, nfiber=nfiber

   on_error,0
   compile_opt idl2
   
   if (~keyword_set(sigma)) then sigma = 1.0
   if (~keyword_set(ncoeff)) then ncoeff = 4
   if (~keyword_set(xmin)) then xmin = 0.0
   if (~keyword_set(xmax)) then xmax = 2047.0
   if (~keyword_set(numbundles)) then numbundles = 16
   ...
   ;----------
   ; Construct a mask that is nonzero only for those pixels within +/- 12 pixels from each arc line center on each fiber.
   ...
   ;----------
   ; Extract arc lines from the [NPIX,NTRACE] image, measuring the wavelength sigmas during extraction for every fiber.
   extract_image, arc_flux, arc_fluxivar*arcmask, xcen, sigma, arclineflux, arclineivar, ansimage=ansimage, wfixed=[1,1], highrej=10, lowrej=10, relative=1, npoly=5
   ...
   nstart = [0,nend[0:numbundles-2]+1]
   
   for iline=0, nline-1 do begin
     for j=0, numbundles-1 do begin

         twidth = reform(width[*,nstart[j]:nend[j]],nline,nfiber[j])
         tmask = reform(gmask[*,nstart[j]:nend[j]],nline,nfiber[j])
        
         ss = where(tmask[iline,*] AND twidth[iline,*] GT 0, ct)
         if (ct GE 0.5*numbundles) then width_bundle[iline,j] = djs_median(twidth[iline,ss]) 
     endfor
   endfor
   ...
   xy2traceset, transpose(xcen), width_final, dispset, inmask=(width_final GT 0), ncoeff=ncoeff, xmin=xmin, xmax=xmax, maxdev=0.2

   ;----------
   ; Compute the widths in each of 4 quandrants on the CCD as the median of the unmasked pixels around the lines being fit

   ; matt modify for 4x4 grid, use quadrupole terms and only half ccd
   ; for quick extract, otherwise do old quadrant fit
   ...
   for i=0,3 do begin
      ...
      if (ct GT 0) then medwidth[i] = median([ (width_fit[x1[i]:x2[i],y1[i]:y2[i]])[indx] ])
   endfor

   splog, 'Median wavelength widths = ' + string(medwidth,format='(4f5.2)') + ' pix (L B T R)' ;left bottom top right
