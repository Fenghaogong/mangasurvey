...
;-
;------------------------------------------------------------------------------
pro spplan2d, topdir=topdir1, run2d=run2d1, mjd=mjd, outdir=outdir,$
 mjstart=mjstart, mjend=mjend, minexp=minexp, clobber=clobber, _extra=foo, NOSPLOG=NOSPLOG

   on_error, 0
   compile_opt idl2
   compile_opt idl2, hidden

   if (~keyword_set(minexp)) then minexp = 1

   ;----------
   ; Determine the top-level of the output directory tree

   if (keyword_set(topdir1)) then topdir = topdir1 else topdir = getenv2('MANGA_SPECTRO_REDUX')
   if ~keyword_set(nosplog) then splog, 'Setting TOPDIR=', topdir
   if (keyword_set(run2d1)) then run2d = strtrim(run2d1,2) else run2d = getenv('RUN2D')
   if ~keyword_set(nosplog) then splog, 'Setting RUN2D=', run2d

   ;----------
   ; Read environment variable for BOSS_SPECTRO_DATA for finding raw data files.

   rawdata_dir = getenv2('MANGA_SPECTRO_DATA')
   if (~keyword_set(rawdata_dir)) then message, 'Must set environment variable BOSS_SPECTRO_DATA'
   if ~keyword_set(nosplog) then splog, 'Setting BOSS_SPECTRO_DATA=', rawdata_dir

   speclog_dir = getenv2('MANGA_SPECTRO_DATA')
   if (~keyword_set(speclog_dir)) then message, 'Must set environment variable SPECLOG_DIR'
   if ~keyword_set(nosplog) then splog, 'Setting SPECLOG_DIR=', speclog_dir
   ...
   if ~keyword_set(nosplog) then splog, 'Number of MJDs = ', nmjd
   ...
   for imjd=0, nmjd-1 do begin
      ...
      if ~keyword_set(nosplog) then splog, ''
      if ~keyword_set(nosplog) then splog, 'Data directory ', inputdir

      ; Find all raw FITS files in this directory
      fullname = spplan_findrawdata(inputdir, nfile)
      if ~keyword_set(nosplog) then splog, 'Number of FITS files found: ', nfile
      ...
      if (nfile GT 0) then begin
         ...
         for i=0, nfile-1 do begin
            ...
            if (size(hdr,/tname) EQ 'STRING') then begin
               ...
               if (strtrim(sxpar(hdr, 'QUALITY'),2) NE 'excellent') then begin
                  if ~keyword_set(nosplog) then splog, 'Warning: Non-excellent quality file ' + fileandpath(fullname[i])
                  FLAVOR[i] = 'unknown'
               endif

               ; Exclude files where the plate number does not match that in the map name
               if (string(PLATEID[i],format='(i4.4)') NE strmid(MAPNAME[i],0,4)) $
                then begin
                  if ~keyword_set(nosplog) then splog, 'Warning: Plate number inconsistent with map name ' + fileandpath(fullname[i])
                  FLAVOR[i] = 'unknown'
               endif

               if (sxpar(hdr, 'MJD') NE thismjd) then if ~keyword_set(nosplog) then splog, 'Warning: Wrong MJD in file '+fileandpath(fullname[i])

               ; MAPNAME should be of the form '0306-51683-01'.
               ; If it only contains the PLATEID (for MJD <= 51454), then find the actual plug-map file.
               if (strlen(MAPNAME[i]) LE 4) then begin
                  plugfile = 'plPlugMapM-' + string(long(MAPNAME[i]), format='(i4.4)') + '-*.par'
                  plugfile = (findfile(filepath(plugfile, root_dir=plugdir), count=ct))[0]
                  if (ct EQ 1) then MAPNAME[i] = strmid(fileandpath(plugfile), 11, 13)
               endif
            endif
         endfor
         ...
         for imap=0, n_elements(allmaps)-1 do begin
            ...
            for iexp=0, n_elements(allexpnum)-1 do begin
               indx = where(MAPNAME EQ allmaps[imap] AND EXPOSURE EQ allexpnum[iexp] AND FLAVOR NE 'unknown', ct)

               if (ct GT 0) then begin
                  spexp1 = spplan_create_spexp(allexpnum[iexp], PLATEID[indx[0]], thismjd, MAPNAME[indx[0]], FLAVOR[indx[0]], EXPTIME[indx[0]], shortname[indx], CAMERAS[indx], minexp=minexp)
            ...
            ;----------
            ; Discard these observations if the plate number is not in the range 1 to 9990.

            if (keyword_set(spexp)) then pltid = long(spexp[0].plateid) else pltid = 0
            if (pltid GT 0 AND pltid LT 9990) then begin
               platestr = string(pltid, format='(i04.4)')
            endif else begin
               if ~keyword_set(nosplog) then splog, 'WARNING: Plate number invalid for MAPNAME=' + allmaps[imap]
               platestr = '0000'
               spexp = 0
            endelse
            ...
            ;----------
            ; Discard these observations if there is not at least one flat, one arc, and one science exposure

            if (keyword_set(spexp)) then begin
               junk = where(spexp.flavor EQ 'flat', ct)
               if (ct EQ 0) then begin
                  if ~keyword_set(nosplog) then splog, 'WARNING: No flats for MAPNAME=' + allmaps[imap]
                  spexp = 0
               endif
            endif
            ...
            if (keyword_set(spexp)) then begin
               ...
               if (ct EQ 0) then begin
                  if ~keyword_set(nosplog) then splog, 'WARNING: No arcs for MAPNAME=' + allmaps[imap]
                  spexp = 0
               endif
            endif
            ...
            if (keyword_set(spexp)) then begin
               ...
               if (ct EQ 0) then begin
                  if ~keyword_set(nosplog) then splog, 'WARNING: No science frames for MAPNAME=' + allmaps[imap]
                  spexp = 0
               endif
            endif
            ...
            if (keyword_set(spexp)) then begin
               ...
               outdir = (n_elements(outdir) eq 0) ? djs_filepath('', root_dir=getenv2('MANGA_SPECTRO_REDUX'), subdir=[run2d,platestr]) : outdir
               ...
               hdr = [hdr, "MJD     " + mjdstr + "  # Modified Julian Date"]
               hdr = [hdr, "RUN2D  " + run2d + "  # 2D reduction name"]
               hdr = [hdr, "planfile2d  '" + planfile + "'  # Plan file for 2D spectral reductions (this file)"]
               hdr = [hdr, "idlspec2dVersion '" + idlspec2d_version() + "'  # Version of idlspec2d when building plan file"]
               hdr = [hdr, "idlutilsVersion '" + idlutils_version() + "'  # Version of idlutils when building plan file"]
               hdr = [hdr, "speclogVersion '" + logvers + "'  # Version of speclog when building plan file"]
               ...
               if (file_test(outdir, /directory) EQ 0) then begin
                  ...
                  if (file_test(outdir)) then message, 'Expecting directory not file '+outdir
                  spawn, 'mkdir -p ' + outdir
               endif
               ; Bad if this exists as an unwriteable dir
               if (file_test(outdir, /directory, /write) EQ 0) then message, 'Cannot write to directory '+outdir

               fullplanfile = filepath(planfile, root_dir=outdir)
               qexist = keyword_set(findfile(fullplanfile))
               if (qexist) then begin
                  if ~keyword_set(nosplog) then $
                      if (keyword_set(clobber)) then splog, 'WARNING: Over-writing plan file: ' + planfile $
                        else splog, 'WARNING: Will not over-write plan file: ' + planfile
               endif
               if ((NOT qexist) OR keyword_set(clobber)) then begin
                  if ~keyword_set(nosplog) then splog, 'Writing plan file ', fullplanfile
                  print, 'Writing plan file ', fullplanfile
