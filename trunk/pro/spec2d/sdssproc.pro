...
pro sdssproc, infile1, image, invvar, indir=indir, $
 ...
 applycrosstalk=applycrosstalk, ccdmask=ccdmask, VISUAL=VISUAL, survey=survey
  ...
  if (NOT keyword_set(expnum)) then message, 'Cannot determine exposure number from file name ' + infile
  ...
  if (~keyword_set(vers2d)) then begin
     ...
     if (~keyword_set(verslog)) then verslog = 'Unknown'
  ...
  sxaddpar, hdr, 'VERSREAD', vers2d, ' Version of idlspec2d for pre-processing raw data', after='VERSUTIL'
  sxaddpar, hdr, 'VERSLOG', verslog[0], ' Version of SPECLOG product', after='VERSREAD'
  sxaddpar, hdr, 'VERSFLAT', versflat[0], ' Version of SPECFLAT product', after='VERSFLAT'
  ...
  if (mjd GE 55415 AND strmatch(camname,'r*')) then sxaddpar, hdr, 'TWOPHASE', 'T' $
    else sxaddpar, hdr, 'TWOPHASE', 'F'
...
if (mjd GE 55052) then begin
   ...
   if (ct EQ 0) then begin
      ...
      if (hartmann EQ 'Left' OR hartmann EQ 'Right') then sxaddpar, hdr, 'QUALITY', 'test' $
      else sxaddpar, hdr, 'QUALITY', 'excellent'
   endif
   if (readimg OR readivar) then begin
      ...
      'b': begin
         ...
         if bossgood then begin
            ...
            bias = [ median(rawdata[10:67,56:2111]), median(rawdata[4284:4340,56:2111]), median(rawdata[10:67,2112:4167]), median(rawdata[4284:4340,2112:4167]) ]
         ; Subtract the bias before computing noise for numerical reasons
            rdnoise = [ djsig(rawdata[10:67,56:2111] - bias[0], sigrej=3.0), djsig(rawdata[4284:4340,56:2111] - bias[1], sigrej=3.0), $
                        djsig(rawdata[10:67,2112:4167] - bias[2], sigrej=3.0), djsig(rawdata[4284:4340,2112:4167] - bias[3], sigrej=3.0) ]
      ...
      'r': begin
         ...
         if bossgood then begin
            if (keyword_set(silent) EQ 0) then splog, 'BOSS survey-quality MJD: applying pixbias whether you like it or not!'
            pp = filepath('', root_dir=getenv2('SPECFLAT_DIR'), subdirectory='biases')
            pixbiasname = findopfile('boss_pixbias-*-'+camname+'.fits*', mjd, pp, silent=silent)
            image = bolton_biassub(rawdata, pp+pixbiasname, rnoise=rnoise, cam=camname, sigthresh=3.0)
            xwid = (size(image))[1]
            ywid = (size(image))[2]
            ...
            bias = [ median(rawdata[10:100,48:2111]), median(rawdata[4250:4340,48:2111]),  median(rawdata[10:100,2112:4175]), median(rawdata[4250:4340,2112:4175]) ]
            ; Subtract the bias before computing noise for numerical reasons
            rdnoise = [ djsig(rawdata[10:100,48:2111] - bias[0], sigrej=3.0), djsig(rawdata[4250:4340,48:2111] - bias[1], sigrej=3.0), $
                        djsig(rawdata[10:100,2112:4175] - bias[2], sigrej=3.0), djsig(rawdata[4250:4340,2112:4175] - bias[3], sigrej=3.0) ]
            rdnoise *= 1.015 * gain ; account for sigma-clipping
            if (keyword_set(silent) EQ 0) then splog, 'Read noise = ', rdnoise, ' electrons'
...
;START OF SDSS 1 - ends on line 1037
  ...
  if (strmatch(obscomm,'*dithered*') OR strmatch(obscomm,'*focus*')) then sxaddpar, hdr, 'QUALITY', 'test'
    
  ;-----------
  ; Check that this was not a non-test exposure taken during the daytime.

  if (keyword_set(silent) EQ 0 AND strmatch(flavor,'science*')) then warn_daytime, hdr
  ...
  if (~readimg AND ~readivar) then return
  ...
  if ((mjd GE 51578 AND mjd LE 51580) AND (camname EQ 'b2' OR camname EQ 'r2') AND (readimg OR readivar)) then begin
    if (camname EQ 'b2') then begin
      i1 = strpos(infile,'b2',/reverse_search)
      if (i1 EQ -1) then message, 'Unable to parse corresponding red file for '+infile
      redfile = infile
      strput, redfile, 'r2', i1
      redfile = (lookforgzip(djs_filepath(redfile, root_dir=indir)))[0]
      
      if (fits_wait(redfile, deltat=1, tmax=1)) then reddata = rdss_fits(redfile, /nofloat, silent=silent) $
      else $
        if (~keyword_set(silent)) then splog, 'Warning: Could not read corresponding red file ' + redfile
    endif else begin
      reddata = rawdata
    endelse
    
    if (keyword_set(reddata)) then ibad = where( reddata[20,*] LT median(reddata[20,*]) - 100 , nbad ) $
    else nbad = 0
      
    if (nbad GT 0) then begin
      if (~keyword_set(silent)) then splog, 'WARNING: Fixing ', nbad, ' shifted rows (from electronics)'
      ...
      for ii=0, nbad-1 do begin
        nshift = (reverse(where(rawdata[20:39,ibad[ii]] GT medval + 100)))[0]
  ...
  if ((mjd GE 51688 AND mjd LT 55025) AND (camname EQ 'b2' OR camname EQ 'r2') AND (readimg OR readivar)) then begin
    if (camname EQ 'b2') then begin
    ...
    if (keyword_set(reddata)) then ibad = where( reddata[20,*] LT median(reddata[20,*]) - 100 AND reddata[2107,*] GT median(reddata[2107,*]) + 100, nbad ) $
    else nbad = 0
      
    if (nbad GT 0) then begin
      if (~keyword_set(silent)) then splog, 'WARNING: Fixing ', nbad, ' dropped-pixel rows (from electronics)'
  ...
  config_dir = filepath('', root_dir=getenv2('MANGADRP_DIR')+getenv2('MANGADRP_VER'), subdirectory='examples')
    
  if (~keyword_set(configfile)) then configfile = findopfile('opConfig*par', mjd, config_dir, /abort_notfound, silent=silent)
  if (~keyword_set(ecalibfile)) then ecalibfile = findopfile('opECalib*par', mjd, config_dir, /abort_notfound, silent=silent)
  if (~keyword_set(bcfile)) then bcfile = findopfile('opBC*par', mjd, config_dir, /abort_notfound, silent=silent)
  ...
  if (naxis1 NE config.ncols OR naxis2 NE config.nrows AND ~keyword_set(silent)) then splog, 'WARNING! Config file dimensions do not match raw image'
    
  qexist = [config.amp0, config.amp1, config.amp2, config.amp3]
  
  ;------
  ; Define the "overscan" regions
  sover = [config.soverscan0, config.soverscan1, config.soverscan2, config.soverscan3]
  nover = [config.noverscan0, config.noverscan1, config.noverscan2, config.noverscan3]
    
  ;------
  ; Define the "mapped overscan" regions
  smapover = [config.smapoverscan0, config.smapoverscan1, config.smapoverscan2, config.smapoverscan3]
  nmapover = [config.nmapoverscan0, config.nmapoverscan1, config.nmapoverscan2, config.nmapoverscan3]
    
  ;------
  ; Define the "overscan rows" (at the bottom of the CCD)
  soverrow = [config.soverscanrows0, config.soverscanrows1, config.soverscanrows2, config.soverscanrows3]
  noverrow = [config.noverscanrows0, config.noverscanrows1, config.noverscanrows2, config.noverscanrows3]
    
  ;------
  ; Data position in the original image
  sdatarow = [config.sdatarow0, config.sdatarow1, config.sdatarow2, config.sdatarow3]
  sdatacol = [config.sdatasec0, config.sdatasec1, config.sdatasec2, config.sdatasec3]
  nrow = [config.ndatarow0, config.ndatarow1, config.ndatarow2, config.ndatarow3]
  ncol = [config.ndatasec0, config.ndatasec1, config.ndatasec2, config.ndatasec3]
    
  ;------
  ; Data position in the final (trimmed) image
  srow = [config.sccdrowsec0, config.sccdrowsec1, config.sccdrowsec2, config.sccdrowsec3]
  scol = [config.sccdcolsec0, config.sccdcolsec1, config.sccdcolsec2, config.sccdcolsec3]
    
  if (naxis2 EQ 2049) then begin
    if (~keyword_set(silent)) then splog, 'WARNING: NROWS is 2049, adjusting config entries'
  ...
  rnoise_expect = [ecalib.readnoiseDN0, ecalib.readnoiseDN1, ecalib.readnoiseDN2, ecalib.readnoiseDN3]
  rnoise_measure = fltarr(4)
  fullWellDN = [ecalib.fullWellDN0, ecalib.fullWellDN1, ecalib.fullWellDN2, ecalib.fullWellDN3]
  ...
  if ((size(image))[0] NE 2) then image = fltarr(nc, nr) $
    else if ((size(image))[1] NE nc OR (size(image))[2] NE nr OR (size(image))[3] NE 4) then image = fltarr(nc, nr)
   ...
   if (exptime GT 640 AND (readimg OR readivar) AND mjd GT 0 AND mjd LT 51570 AND flavor NE 'bias' AND flavor NE 'dark') then qshutter = 1
    ...
    if (qexist[iamp] EQ 1) then begin
      if (readimg OR readivar) then begin
        if (nover[iamp] NE 0) then begin
          ...
          biasreg = rawdata[sover[iamp]:sover[iamp]+nover[iamp]-1, sdatarow[iamp]:sdatarow[iamp]+nrow[iamp]-1]
        endif else if (nmapover[iamp] NE 0) then begin
          ; Use the "mapped overscan" region
          biasreg = rawdata[smapover[iamp]:smapover[iamp]+nmapover[iamp]-1, sdatarow[iamp]:sdatarow[iamp]+nrow[iamp]-1]
        endif
        ...
        if (~keyword_set(silent)) then begin
          splog, 'Measured read-noise in DN for amp#', iamp, ' = ', rnoise_measure[iamp]
        ...
        if (rnoise_measure[iamp] GT rnoise_expect[iamp]+1.0) then $
          splog, 'WARNING: Amp #', iamp, ' expected read noise = ', rnoise_expect[iamp],  ', measured = ', rnoise_measure[iamp], ' DN', format='(a,i1,a,f5.2,a,f8.2,a)'
        ...
        for siglevel=1,3 do begin
          ...
          biasdiff = biasreg[isort[long(nn*(0.5+0.5*fnormal))]]  - biasreg[isort[long(nn*0.5*(1.-fnormal))]]
          ...
          if (qbiaslo OR qbiashi) then $
            splog, 'WARNING: Amp #', iamp, ' bias region difference at ', 100.*fnormal, 'th-percentile = ', biasdiff, ' DN, expected ', 2*siglevel*rnoise_expect[iamp], format='(a,i1,a,f5.2,a,f7.1,a,f5.1)'
        endfor
        ...
        lopixval = rawdata[sdatacol[iamp]:sdatacol[iamp]+ncol[iamp]-1, sdatarow[iamp]+5:sdatarow[iamp]+nrow[iamp]-1] LT lovalue
        numlopix = total(lopixval)
        fraclopix = numlopix / n_elements(lopixval)
        lopixval = 0 ; clear memory
        if (~keyword_set(silent)) then $
          splog, 'Amp #', iamp, ' Number of pix below bias-5*sigma=', lovalue, ' DN is ', numlopix, format='(a,i1,a,f6.0,a,i7)'
        if (100*fraclopix GT 0.01) then $
          splog, 'WARNING: Amp #', iamp, ' way too many pixels (', 100*fraclopix, '%) below bias-5*sigma=', lovalue, ' DN', format='(a,i1,a,f6.2,a,f6.0,a)'
        ...
        image[scol[iamp]:scol[iamp]+ncol[iamp]-1, srow[iamp]:srow[iamp]+nrow[iamp]-1] = $
          (rawdata[sdatacol[iamp]:sdatacol[iamp]+ncol[iamp]-1, sdatarow[iamp]:sdatarow[iamp]+nrow[iamp]-1] - biasval) * gain[iamp]
          
        ; Add to the header
        sxaddpar, hdr, 'GAIN'+string(iamp,format='(i1)'), gain[iamp], ' Gain in electrons per ADU'
        sxaddpar, hdr, 'RDNOISE'+string(iamp,format='(i1)'), gain[iamp]*rnoise_measure[iamp], ' Readout noise in electrons'
      endif
    endif
  endfor
  ...
  if (qshutter) then begin
    if (~keyword_set(silent)) then splog, 'WARNING: Correcting for open shutter during readout '
  ...
  if (readivar) then begin
    if ((size(invvar))[0] NE 2) then invvar = fltarr(nc, nr) $
      else if ((size(invvar))[1] NE nc OR (size(invvar))[2] NE nr OR (size(invvar))[3] NE 4) then invvar = fltarr(nc, nr)
  ...
  for iamp=0, 3 do begin
    if (qexist[iamp] EQ 1) then begin
      ...
      satmask[scol[iamp]:scol[iamp]+ncol[iamp]-1, srow[iamp]:srow[iamp]+nrow[iamp]-1] = $
         rawdata[sdatacol[iamp]:sdatacol[iamp]+ncol[iamp]-1, sdatarow[iamp]:sdatarow[iamp]+nrow[iamp]-1] LT fullWellDN[iamp]
        
      admask[scol[iamp]:scol[iamp]+ncol[iamp]-1, srow[iamp]:srow[iamp]+nrow[iamp]-1] = $
        rawdata[sdatacol[iamp]:sdatacol[iamp]+ncol[iamp]-1, sdatarow[iamp]:sdatarow[iamp]+nrow[iamp]-1] EQ 65535
        
      ; Flux term below
      expr1 = abs(image[scol[iamp]:scol[iamp]+ncol[iamp]-1, srow[iamp]:srow[iamp]+nrow[iamp]-1])
      ...
      if (qshutter) then expr2 = abs(smearimg[scol[iamp]:scol[iamp]+ncol[iamp]-1, srow[iamp]:srow[iamp]+nrow[iamp]-1]) $
        else expr2 = 0
      ...
      ; Add a noise term of 100 ADU to the left amplifier of r2
      if (mjd EQ 51779 AND camname EQ 'r2' AND iamp EQ 2) then expr4 = expr4 + 100.^2
        
      invvar[scol[iamp]:scol[iamp]+ncol[iamp]-1, srow[iamp]:srow[iamp]+nrow[iamp]-1] = 1.0/(expr1 + expr2 + 0.01 * expr2^2 + expr3 + expr4)
  ...
  if (nblead GT 0) then begin
    if (~keyword_set(silent)) then splog, 'Number of bleading columns = ', nblead
  ...
  if (arg_present(nsatrow)) then begin
    ...
    if (~keyword_set(silent)) then splog, 'Number of saturated rows = ', nsatrow
  endif
  ...
  ;------
  ; Mask out bad columns
  bcmask = make_badcolumn_mask(filepath(bcfile,root_dir=config_dir), camrow, camcol, silent=silent, nr=nr,nc=nc)
...
if (arg_present(ccdmask) AND readimg) then ccdmask = lonarr(size(image,/dimens))
...
if (keyword_set(applybias) AND readimg AND (bossgood eq 0)) then begin
  ...
  pixbiasname = findopfile('pixbiasave-*-'+camname+'.fits*', mjd, pp, silent=silent)
  if (~keyword_set(pixbiasname)) then pixbiasname = findopfile('pixbias-*-'+camname+'.fits*', mjd, pp, silent=silent)
    
  if (~keyword_set(pixbiasname)) then begin
    if (~keyword_set(silent)) then splog, 'WARNING: Bias image not found for camera ' + camname
  endif else begin
    if (~keyword_set(silent)) then splog, 'Correcting with bias image ' + pixbiasname
    pixbiasimg = mrdfits(djs_filepath(pixbiasname, root_dir=pp), silent=silent)
      
    if (total(size(pixbiasimg,/dimens) NE size(image,/dimens)) GT 0) then message, 'Dimensions of image and bias image differ!'
