...
;-
;------------------------------------------------------------------------------
pro divideflat, flux, fflat, invvar=invvar, minval=minval, quiet=quiet, VISUAL=VISUAL, SURVEY=survey, PREDIV=PREDIV
  ...
  compile_opt idl2
   ...
   npix = dims[0]
   if (ndim EQ 1) then ntrace = 1 else ntrace = dims[1] 

   if (n_elements(minval) EQ 0) then minval = 0.03

   if (keyword_set(invvar) AND (total(size(invvar,/dimens) NE dims) NE 0)) then message, 'FLUX and FLUXIVAR are not the same dimensions'

   if (total(size(fflat,/dimens) NE dims) NE 0) then message, 'FLUX and FFLAT are not the same dimensions'

   prediv=flux
   for itrace=0, ntrace-1 do begin

      ;  Do we really need to reject bad fibers here, does it hurt to divide them out anyway???
      ...
      if (1) then begin
         ...
         if (ngood GT 0) then begin
            ...
            flux[igood,itrace] = flux[igood,itrace] / fflat[igood,itrace]
            if (keyword_set(invvar)) then invvar[igood,itrace] = invvar[igood,itrace] * fflat[igood,itrace]^2
         endif else begin
            if keyword_set(quiet) EQ 0 then splog, 'No good flat field points in trace ', itrace
         endelse

         if (nbad GT 0) then begin
            flux[ibad,itrace] = 0.0
            if (keyword_set(invvar)) then invvar[ibad,itrace] = 0.0
            if (nbad GT 200 AND (keyword_set(quiet) EQ 0)) then splog, 'Warning: Reject ', nbad, ' low points in trace ', itrace
         endif

      endif else begin ; BAD FIBER

         flux[*,itrace] = 0.0
         if (keyword_set(invvar)) then invvar[*,itrace] = 0.0
