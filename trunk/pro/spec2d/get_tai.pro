...
pro get_tai, hdr, tai_beg, tai_mid, tai_end, silent=silent
   ...
   qscience = strmatch(sxpar(hdr, 'FLAVOR'), 'science*') OR strmatch(sxpar(hdr, 'FLAVOR'), 'smear*')

   if (tai_beg GT 4.0d9 AND tai_end GT 4.0d9 AND tai_end - tai_beg GE 0) then begin
      tai_mid = (tai_end + tai_beg)/2.0
      exptime_guess = tai_end - tai_beg
      ; The following condition is possible if the exposure was paused for more than 2 minutes.
      if (exptime_guess GT exptime + 120 AND qscience AND keyword_set(silent) EQ 0) then splog, "Warning: (TAI-END) - (TAI-BEG) > EXPTIME + 120 sec"
      ; The following should never happen.
      if (exptime_guess LT exptime - 20 AND keyword_set(silent) EQ 0) then splog, "Warning: (TAI-END) - (TAI-BEG) < EXPTIME - 20 sec"
      ...
      tai_mid = 0
      if (~keyword_set(silent)) then splog, 'WARNING: TAI,TAI-BEG,TAI-END all appear to be invalid!'
   endelse
