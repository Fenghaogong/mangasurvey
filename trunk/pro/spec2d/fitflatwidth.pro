...
function fitflatwidth, flux, fluxivar, ansimage, fibermask, $
 ...
 inmask=inmask, double=double, quick=quick, nbundle=nbundle, nfiber=nfiber
   ...
   if (~keyword_set(mask)) then mask = (flux GT 0) * (fluxivar GT 0) 
   ...
   width = make_array(size=size(flux), /float)  ;created here
   if (igood[0] NE -1) then width[igood] = (1 + widthterm[igood] / flux[igood])

   ndim = size(sigma, /n_dimen)
   if (n_elements(sigma) EQ 1) then begin
      width = width * sigma[0]              ;populate width with the initial sigma guess 
   endif else if (ndim EQ 1) then begin
      for itrace=0, ntrace-1 do width[*,itrace] = width[*,itrace] * sigma[itrace]
   endif else if (ndim EQ 2) then begin
      if (n_elements(sigma) NE n_elements(width)) then message, 'Dimensions of SIGMA and WIDTH do not agree'
      width = width * sigma
   endif else begin
   ...
   ;----------
   ; Perform median across bundles on good arclines only; somewhat tedious, but it works
   width_bundle = fltarr(nrow,numbundles)
   
   ;start/stop points of a block
   nend = total(nfiber,/cumul)-1
   nstart = [0,nend[0:nbundle-2]+1]

   for irow=0, nrow-1 do begin ;loop through rows
      for j=0, numbundles-1 do begin ;loop through bundles
        
         twidth = reform(width[*,nstart[j]:nend[j]],nrow,nfiber[j])
         tmask = reform(mask[*,nstart[j]:nend[j]],nrow,nfiber[j])
        
         ss = where(tmask[irow,*], ct)
         ; At least half the points should be good
         if (ct GE 0.5*nfiber[j]) then width_bundle[irow,j] = djs_median(twidth[irow,ss]) $
            else width_bundle[irow,j] = 0
      endfor
   endfor

   ml_expand, width_bundle, nfiber, ntrace, width=width_final

   ;----------
   ; Turn the widths back into a traceset.

   ; Generate the corresponding mask that is the same within each bundle, and marked as good if at least 25% of the points are unmasked
   if (keyword_set(inmask) NE 0) then begin
      mask_bundle = congrid(float(inmask), nrow, numbundles,/interp) GE 0.25
      ml_expand, mask_bundle, nfiber, ntrace, width=mask_final
   endif else begin
      mask_final = width_final GT 0
   endelse

   xy2traceset, findgen(nrow) # replicate(1,ntrace), width_final, widthset, ncoeff=ncoeff, xmin=xmin, xmax=xmax, inmask=mask_final, double=double

   ;----------
   ; Compute the widths in each of 4 regions on the CCD as the median of the unmasked pixels
   ...
   splog, 'Median spatial widths = '  + string(medwidth,format='(4f5.2)') + ' pix ' + comment
