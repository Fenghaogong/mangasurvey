...
function match_trace, image, invvar, xcen, xpoly=xpoly, ypoly=ypoly, $
   first=first, maxiter=maxiter, radius=radius, nfiber=nfiber

   on_error, 0
   compile_opt idl2
   
  if ~keyword_set(xpoly) then xpoly=3
  if ~keyword_set(ypoly) then ypoly=3
  if ~keyword_set(maxiter) then maxiter=10
  if ~keyword_set(radius) then radius=2
  ...
  tmp_xpos = ml_trace_fweight(image, xcen, ycen, invvar=invvar,radius=radius, nfiber=nfiber)
  tmp_xpos = ml_trace_fweight(image, tmp_xpos, ycen, invvar=invvar,radius=radius, nfiber=nfiber)
  tmp_xpos = ml_trace_fweight(image, tmp_xpos, ycen, invvar=invvar,radius=radius, nfiber=nfiber)
  first = ml_trace_fweight(image, tmp_xpos, ycen, xerr=errfirst, invvar=invvar,radius=radius, nfiber=nfiber)
  diff = first - xcen 
...
;	let's do y first
  ...
  for iiter=0, maxiter - 1 do begin
    ...
    outmask = 0
    qdone = djs_reject(diff, shift, outmask=outmask, invvar=ivar,upper=8,lower=8)
    ivar = ivar * outmask

    bbad = where(outmask EQ 0, nbad)
    sumbad = sumbad + nbad
    splog, "Iteration: ",iiter, " Pixels Rejected: ", nbad, " Finished? ", qdone, format='(a,i3,a,i6,a,i2)'
    if qdone EQ 1 then iiter = maxiter
  endfor

  if sumbad GT 20000L then splog, 'Warning: Large number of pixels rejected! ', sumbad
