...
pro extract_image, fimage, invvar, xcen, sigma, flux, finv, yrow=yrow, $
               ...
               pimage=pimage, VISUAL=VISUAL, SURVEY=survey
...
; Prepare Output arrays
    if ((size(flux))[0] NE 2) then flux = fltarr(nRowExtract, nTrace) $
     else if ((size(flux))[1] NE nRowExtract OR (size(flux))[2] NE nTrace OR (size(flux))[3] NE 4) then flux = fltarr(nRowExtract, nTrace)

    if ((size(finv))[0] NE 2) then finv = fltarr(nRowExtract, nTrace) $
     else if ((size(finv))[1] NE nRowExtract OR (size(finv))[2] NE nTrace OR (size(finv))[3] NE 4) then finv = fltarr(nRowExtract, nTrace)
...
; Check to see if keywords are set to fill optional arrays
   ...
   splog, 'LOOPING THROUGH NUMBER OF ROWS TO EXTRACT ----------------'
   print, ' ROW NITER SIG(med) CHI^2'
   for iy=0, nRowExtract-1 do begin
     cur = yrow[iy]
     
     print, 'EXTRACTING '+strtrim(cur,2)+' row!'
     ...
     pixelmasktemp = 0
     ansrow = extract_row(fimage[*,cur], invvar[*,cur], xcen[cur,*],sigmacur[*],ymodel=ymodelrow, fscat=fscatrow, proftype=proftype, $
                          iback=iback, reject=reject, pixelmask=pixelmasktemp, wfixed=wfixed, mask=masktemp, diagonal=prow, nPoly=nPoly, $
                          niter=niter, squashprofile=squashprofile,inputans=inputans,  maxIter=maxIter, highrej=highrej, lowrej=lowrej, whopping=whoppingcur, $
                          relative=relative, oldreject=oldreject, reducedChi=chisqrow, nband=nband, contribution=contribution, visual=visual, survey=survey)

     mask[*,cur] = masktemp

     if (total(finite(ansrow) EQ 0) GT 0) then message, 'ABORT! ansrow has NaNs at row ' + strtrim(cur,2)
     if (total(finite(ymodelrow) EQ 0) GT 0) then message, 'ABORT! ymodelrow has NaNs at row ' + string(cur,2)
     if (total(finite(fscatrow) EQ 0) GT 0) then message, 'ABORT! fscatrow has NaNs at row ' + string(cur,2)

     if(ARG_PRESENT(ymodel)) then ymodel[*,cur] = ymodelrow
     if(ARG_PRESENT(fscat)) then fscat[iy,*] = fscatrow
     chisq[cur] = chisqrow

     calcflux, ansrow, prow, fluxrow, finvrow, wfixed, proftype, lTrace,nCoeff,pixelmasktemp, squashprofile=squashprofile
     flux[iy,*] = fluxrow 
     finv[iy,*] = finvrow
     ...
     if(ARG_PRESENT(pixelmask)) then begin
       ...
       ;---------------------------------------------------
       ; Now attempt a cross-talk flag for whopping terms do we need a flag for regular profiles, or just test the same??
  ...
  if keyword_set(VISUAL) then begin
    getwindow,/open
    ml_tvimage, flux, /axis, axkeywords={charsize:2,xtitle:'x pix (row)',ytitle:'Fiber Number',title:'Extracted Flat Flux'}
  endif
