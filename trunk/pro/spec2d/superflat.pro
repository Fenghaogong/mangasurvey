...
function superflat, flux, fluxivar, wset, x2=x2, $
 ...
 nord=nord, npoly=npoly, upper=upper, lower=lower, VISUAL=VISUAL

  on_error,0
  compile_opt idl2
  
   if (~keyword_set(minval)) then minval = 0.0
   if (~keyword_set(nord)) then nord = 4
   ...
   ; There must be at least one unmasked pixel at each included wavelengths, and it must at least fall on the CCD for all good fibers.
   goodpix = where((fluxivar GT 0) AND transpose(rebin(fibermask,ntrace,ny)) EQ 0)
   ...
   ; Do this with a straight mean value for all wavelengths in common, interpolating over bad pixels.
   ...
   for i=0, ntrace-1 do begin
      ...
      if (ntmp GT filtsz) then $
       medval[i] = median([ median(tmpflux[ (filtsz-1)/2 : ntmp-(filtsz-1)/2 ], filtsz) ]) $
      else $
       medval[i] = median([tmpflux])
   endfor

   ;------
   ; Limit the superflat to use only good fibers, and those fibers that have at least 95% good wavelength range as compared to the best fiber
   ; and whose counts are within 30% of the median (good) fiber throughput.
   globalmed = median([medval[igood]])

   if (globalmed LT 0) then message, 'Median flat-field vector is negative!'
   igood = where(fibermask EQ 0 AND fracpts GE 0.95*max(fracpts[where(fibermask EQ 0)]) AND abs(medval-globalmed)/globalmed LT 0.30, ngood)
   ...
   ;------
   ; Create a version of flux (and fluxivar) that has all fibers approximately scaled to have a median value of 1
   scalef = fltarr(ny,ntrace)
   scalefivar = fltarr(ny,ntrace)
   for i=0, ntrace-1 do scalef[*,i] = flux[*,i] / medval[i]
   for i=0, ntrace-1 do scalefivar[*,i] = fluxivar[*,i] * (medval[i])^2
   ...
   indx=where((flux[*,igood])[isort] gt minval)
   if (keyword_set(x2)) then thisx2 = allx2[indx]
   if (indx[0] EQ -1) then message, 'No points above MINVAL'

   ;x2 is off, nord=4, npoly undef, ngood varies, 
   sset = bspline_iterfit(allwave[indx], allflux[indx], invvar=allivar[indx], x2=thisx2, nord=nord, npoly=npoly, everyn=ngood, maxiter=maxiter, upper=upper, lower=lower, outmask=mask, requiren=2)
...
; Should move this plotting elsewhere ???
   ...
   if (keyword_set(title)) then begin
      ; Interpolate all the spectra to the wavelength mapping of the central fiber
      wplot = loglam[*,ntrace/2] ;central fiber
      fplot = fltarr(ny,ntrace)
      for i=0L, ntrace-1L do begin
         ii = where(fluxivar[*,i] GT 0, ct)
         if (ct GT 0) then fplot[*,i] = interpol(flux[*,i], loglam[*,i], wplot)
      endfor
      ...
      wrange = minmax( sset.fullbkpt[sset.nord-1:n_elements(sset.fullbkpt)-sset.nord+1] )
      ...
      djs_plot, 10.^wrange, yrange, /nodata, xrange=10.^wrange, xstyle=1, yrange=yrange, ystyle=1, thick=2, xtitle='\lambda [A]', ytitle='Normalized flux', title=title
      for k=0,n_elements(percentiles)-1 do djs_oplot, 10.^wplot, fmed[*,k]
      djs_oplot, 10.^wplot, plot_fit, color='red'

      djs_xyouts, total([0.95,0.05]*!x.crange), total([0.05,0.95]*!y.crange), 'RED = Superflat fit to central fiber', color='red'
      djs_xyouts, total([0.95,0.05]*!x.crange), total([0.10,0.90]*!y.crange), 'BLACK =' + string(percentiles*100, format='('+string(n_elements(percentiles))+'f6.1)')+' percentiles'
   endif
