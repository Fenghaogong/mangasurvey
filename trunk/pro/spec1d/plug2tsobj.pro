...
;-
;------------------------------------------------------------------------------
function plug2tsobj, plateid, ra, dec, mjd=mjd, indir=indir1, dmin=dmin, silent=silent

  on_error,0
  compile_opt idl2
  compile_opt idl2, hidden
  
   if (n_elements(plateid) NE 1) then message, 'PLATEID must be a scalar!'
   if keyword_set(mjd) then $
      if (n_elements(mjd) NE n_elements(plateid)) then message, 'Number of elements in PLATEID and MJD must agree!'
      

   platestr = string(plateid, format='(i4.4)')
   if (n_elements(indir1) GT 0) then indir = indir1 $
    else indir = getenv2('MANGA_SPECTRO_REDUX')+getenv2('RUN2D')+platestr

   if keyword_set(ra) then $
      if (n_elements(ra) NE n_elements(dec)) then message, 'Number of elements in RA and DEC must agree!'

   if (~keyword_set(dmin)) then dmin = 2.0
   ...
   if (keyword_set(filename) EQ 0) then begin
      ...
      if keyword_set(filename) then qsorted = 0B
   end
   ...
   if (~keyword_set(tstemp)) then begin
   ...
   for iplug=0, n_elements(ra)-1 do begin
      ...
      if (ra[iplug] NE 0 AND dec[iplug] NE 0) then begin
         adist = djs_diff_angle(tstemp.matchra, tstemp.matchdec, ra[iplug], dec[iplug])
         ...
         if (thismin GT dmin/3600.) then begin
            if keyword_set(plugmap) then begin
               if (strmatch(plugmap[iplug].objtype,'SKY*') EQ 0) then $
                splog, 'Warning: Unmatched OBJTYPE=' + strtrim(plugmap[iplug].objtype,2) + ' at RA=', ra[iplug], ',DEC=', dec[iplug]
            endif else begin
                splog, 'Warning: Unmatched ' + ' at RA=', ra[iplug], ',DEC=', dec[iplug]
            endelse
         endif else begin
