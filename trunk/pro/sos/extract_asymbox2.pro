...
;-
;------------------------------------------------------------------------------
function extract_asymbox2, image, left, right, ycen, weight_image=weight_image, f_ivar=f_ivar, model=model
  ...
  compile_opt idl2
   ...
   if (N_params() LT 2) then begin
      print, 'Syntax - fextract = extract_idlcar( image, left, right, [ycen,  weight_image=weight_image, f_ivar=f_ivar, model=model] )'
      return, -1
   endif

   if ~keyword_set(left) then message, 'LEFT is required'
   if ~keyword_set(right) then message, 'RIGHT is required'
   if (N_elements(LEFT) NE N_elements(RIGHT)) then message, 'Number of elements in LEFT and RIGHT must be equal'
   ...
   if (N_elements(LEFT) NE N_elements(YCEN)) then message, 'Number of elements in LEFT and YCEN must be equal'
...
;   if (min(ycen) LT 0 OR max(ycen) GT ny-y) then message, 'YCEN contains values out of range'
   ...
   bigleft = 0.
   bigright = 0.
   weight = (((fracleft+fracright) > 0) < 1)  * ((spot GE -0.5) AND (spot LT (nx-0.5))) * ((bigy GE 0) AND (bigy LE (ny-1)))
   spot = 0.
   fracleft = 0.
   fracright = 0.
   ...
   if keyword_set(weight_image) then begin
     fextract =total(reform(weight * weight_image[fullspot,bigy] * image[fullspot,bigy],npix,nTrace,tempx),3)
     f_ivar   =total(reform(weight * weight_image[fullspot,bigy], npix,nTrace,tempx),3)
   ...
   if ARG_PRESENT(model) then begin
       for i=0,tempx-1 do begin
        model[fullspot[*,i],bigy[*,i]] = model[fullspot[*,i],bigy[*,i]] + fextract*weight[*,i]
        modelwt[fullspot[*,i],bigy[*,i]] = modelwt[fullspot[*,i],bigy[*,i]] + weight[*,i]
       endfor
