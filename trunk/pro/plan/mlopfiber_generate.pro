...
;
; CALLING SEQUENCE:
;   mlopfiber_generate, plate, mjd=, [ camname=, expnum= ]
;
; INPUTS:
...
;-
;------------------------------------------------------------------------------
pro mlopfiber_generate, plate, mjd=mjd, expnum=expnum1, camname=camname1
  ...
  compile_opt idl2, hidden
   ...
   if (n_elements(mjd) NE nplate) then message, 'PLATE and MJD must have same number of elements!'

   ; If PLATE is a vector, then call recursively
   if (nplate GT 1) then begin
      for iplate=0L, nplate-1L do begin
         mlopfiber_generate, plate[iplate], mjd=mjd[iplate], camname=camname1
      endfor
      return
   endif
   ...
   if (keyword_set(camname)) then camname = camname1 else camname = ['b1','b2','r1','r2']

   readspec, plate, mjd=mjd, objhdr=objhdr
   if (~keyword_set(objhdr)) then message, 'spPlate file not found for plate '+string(plate)+ ' MJD '+string(mjd)
   idlist = sxpar(objhdr, 'EXPID*')
   cartid = sxpar(objhdr, 'CARTID')
   totfiber = sxpar(objhdr, 'NAXIS2')
   nhalf = totfiber / 2
   ...
   for icam=0, n_elements(camname)-1 do begin
      ...
      filename = 'spCFrame-'+camname[icam]+'-'+string(expnum,format='(i8.8)') + '.fits'
      fullname = filepath(filename, root_dir=getenv('MANGA_SPECTRO_REDUX'), subdir=[getenv('RUN2D'), string(plate,format='(i4.4)')])
      ...
      if (max(ximg) LT 1000) then begin
         ...
         fullflatname = filepath(flatname, root_dir=getenv('MANGA_SPECTRO_REDUX'), subdir=[getenv('RUN2D'), string(plate,format='(i4.4)')])
      ...
      if (~keyword_set(ximg)) then message, 'File not found: '+fullname
      ...
      fiberparam[icam].totfiber = nhalf
      nbundle = nhalf / 20
      for j=0, nbundle-1 do fiberparam[icam].fiberspace[j] = median(xdiff[j*20:j*20+18])
      fiberparam[icam].bundlegap[0] = xpos[0] ; XSTART
      for j=1, nbundle-1 do fiberparam[icam].bundlegap[j] = xpos[j*20] - xpos[j*20-1] - fiberparam[icam].fiberspace[j]
   endfor

   ; Append this to the opFibers.par file
   opfile = filepath('opFibers.par', root_dir=getenv('IDLSPEC2D_DIR'), subdir='opfiles')
