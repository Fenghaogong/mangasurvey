...
pro bundlefluxrationew, plate, mjd, expseq,totresult,bundleids=bundleids,centerwave=centerwave,dwave=dwave,staroffx=staroffx,staroffy=staroffy;, newloglam,mratfit,factor=factor,scalebest=scalebest,rotationbest=rotationbest,xbest=xbest,ybest=ybest
;common com_fluxbundle, cube,xarr,yarr,wavearr
common com_fluxbundle2, psfplane,xarr,wavearr
...
;    bundleid = 'ma001'
  ...
  if NOT keyword_set(bundleids) then bundleids=slitmap[all[uu]].ifuname
  if plate eq 6870 then bundleids=bundleids[where(bundleids ne 'ma001')]
  if plate eq 6655 then bundleids=bundleids[where(bundleids eq 'ma003')]
...
;    endfor
    ...
    bluepart = where(10^newloglam gt 3700 and 10^newloglam lt 6000)
    fiberfluxblue = total(totfluxarr[bluepart,*,*]*totivararr[bluepart,*,*],1)/total(totivararr[bluepart,*,*],1)
...
;    innermask = (sqrt(fiberx*fiberx+fibery*fibery) lt 3) and bmap.gbu ge 0
    ...
    if innermask[reffiber] eq 0 then begin 
      print,'Warning: reffiber is not one of the inner 7 fibers.'
      print,'You could manually set innermask, innerfiber.'
      stop
    endif
...
;    fluxarr = fluxarr - skyresi#(fltarr(nfiber+1))
    ...
    reffluxarr = fltarr(nlam,nexp)
    refivararr = reffluxarr & refdisparr=reffluxarr
    for ie=0,nexp-1 do begin
      tmp = max(fiberfluxblue[innerfiber,ie],ref_i)
      reffluxarr[*,ie] = sbfluxarr[*,innerfiber[ref_i],ie]
      refivararr[*,ie] = sbivararr[*,innerfiber[ref_i],ie]
      refdisparr[*,ie] = totdisparr[*,innerfiber[ref_i],ie]
    endfor
 
    comflux = total(reffluxarr,2)/nexp
    var = total(1/refivararr,2)/nexp^2
    comivar = (finite(var))/var
    disparr = total(refdisparr,2)/nexp
...
;    if nbad gt 0 then comivar[t] = 0.0
    ...
    tmp = typingmodule(comflux,newloglam,comivar,disparr,bind[reffiber],observations[0],plugmap,kindx=kindx,modflux=modflux)
    ...
    refwave = 5300.
    for ie=0,nexp-1 do begin
      ;reset reffiber for each exposure.
      tmp = max(fiberflux[innerfiber,ie],refwithin)
      reffiber = innerfiber[refwithin]
...
;    chi2sizearr=fltarr(nscl,nsize); chi2sizearr = fltarr(nwave,nsize)
      ...
      for is=0,nsize-1 do begin
        ...
        if is eq 0 then status=mldar((observations[ie].ha-(slitmap[bind[0]].ra-slitmap[bind[0]].cenra))/15.,plugmap[bind[0]].dec,wavearr,parangle,offsetx0,offsety0,waveref=refwave,raobj=slitmap[bind[0]].ra,racen=slitmap[0].cenra,deccen=slitmap[0].cendec,/distort)
        n_wavearr = n_elements(wavearr)
...
;                  rotation=rotationbest,nstep=2000)
        fchi2[is]=mcmcbundleratio(fluxratio[*,innerfiber],$
                  ...
                  weight=modflux*init_thru,ivarweight=sbivararr[*,innerfiber,ie],$
                  lambda=10^newloglam,xbest=xbest,ybest=ybest,scale=scalebest,$
                  rotation=rotationbest,nstep=2000)
...
;   ycen_final = ycen_arr[ind_s]
        ...
        bestchi2=mcmcbundleratio(fluxratio[*,innerfiber],fluxratioerr[*,innerfiber], $
           ...
           ivarweight=sbivararr[*,innerfiber,ie],lambda=10^newloglam,xbest=xbest,$
      ...
      newchi = matchbundleratio2(fluxratio[*,innerfiber],fluxratioerr[*,innerfiber],(where(innerfiber eq reffiber))[0],distarr[innerfiber,*],cenwave=centerwave,dwave=dwave,usemask=intarr(ninner)+1,weight=modflux*init_thru,ivarweight=sbivararr[*,innerfiber,ie],lambda=10^newloglam,predict=predict)
stop
      example = [0,3,6]
      psplot,'fluxratio_example.ps',/square,/color
      !p.multi=[0,3,3]
      psset,/multi
      for iwave=0,n_elements(example)-1 do begin &$
         plotratiofit,fiberoffx[innerfiber],fiberoffy[innerfiber],string(innerfiber+1,format='(i0.0)'),string(fluxratio[example[iwave],innerfiber]*100,format='(f0.2)'),string(predict[example[iwave],*]*100,format='(f0.2)'),string(fluxratioerr[example[iwave],innerfiber]*100,format='(f3.1)'),titles=['Fiber Num','Data','Best-fit Model'],color=1,charsize=1. &$
         oplot,-xbest+offsetx,-ybest+offsety &$
         useless = min(abs(wavearr-centerwave[example[iwave]]),id_w) &$
         plots,-xbest+offsetx[id_w],-ybest+offsety[id_w],ps=1,symsize=2 &$
...
;      covfn = modelthrupt(distarr[innerfiber[bright],*],wavearr)
      ...
      snratio = sbfluxarr[*,innerfiber,ie]*sqrt(sbivararr[*,innerfiber,ie])
      snratiobin = rebin(snratio,11,ninner)
      snratiobinr = snratiobin/(snratiobin[*,where(innerfiber eq reffiber)]#(fltarr(ninner)+1))
      select = snratiobinr gt (sqrt(2.)-1)
      selectbig = rebin(select,nlam,ninner,/sample)
      data = total(sbfluxarr[*,innerfiber,ie]*selectbig,2)
      var = total((1/sbivararr[*,innerfiber,ie])*selectbig,2)
      ivar = 1/var

      ivarprep = sbivararr[*,innerfiber,ie]
      ind = where(select eq 0) 
      ivarprep[ind] = 1.0
      ivarproduct = product(ivarprep,2)
      ivar[where(ivarproduct eq 0)] = 0.0
 
      covfn = modelthrupt(distarr[innerfiber,*],wavearr)
      covfnbig = fltarr(nlam,ninner)
      for i=0,ninner-1 do covfnbig[*,i] = interpol(covfn[*,i],wavearr,10^newloglam,/spline)
      totcovfn = total(covfnbig*selectbig,2)
