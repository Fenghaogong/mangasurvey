FUNCTION mlmeanclip, Image, result_mean, result_sigma, CLIPSIG=clipsig, MAXITER=maxiter, $
...
;                   improved doc.  RSH
;-
; Modified 6/14/07: Tweaked MEANCLIPDRL to ignore 0-values when it
...
; *all* values with rejection.
  ...
  medval=median_drl(subset_skpix)
  sig=stdev(subset_skpix)
  wsm=where(abs(skpix-medval) LE clipsig*sig,ct)
  subs=subs[wsm]
