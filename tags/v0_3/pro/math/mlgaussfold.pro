; Function mlgaussfold
;
; A hack of the standard gaussfold routine for use with MaNGA data
...
;
; Kludged by DRL on Oct 17 2011 for short spectra
;-


FUNCTION mlgaussfold, lam, flux, fwhm, lammin=lammin, lammax=lammax
   ...
   ; DRL kludged this next line so that the routine doesn't fail
   ; when 17x the FWHM is larger than the input spectrum length
   ; Kludged Oct 17 2011
   IF (window gt (size(lam))[1]) then window=(size(lam))[1]
