...
pro manga3d, input=input, outname=outname, inspect=inspect, quick=quick, doall=doall
...
for i=0,nexp-1 do begin
  ...
  status=mlimportspec(wave,redx[i],obsparam,slitmap,bmap,spec,xinbundle,yinbundle,fstat,filepath,flavor,quick=quick,doall=doall)
  ...
  ; If we were doing flux calibration, this would be a good place for it
  ; see spcombine_v5.pro and spcoadd_v5.pro for the BOSS version.
...
writefits,outpath+'CovMap.fits',CovMap
...
for k=0,nwave-1 do begin
  ...
  badloc=where(dat eq 0.)
  if (size(badloc))[0] ne 0 then mask[badloc]=1
  bigmask[k,*]=mask[*]

  interpimage=mlcalifainterp(x,y,dat,mask,[Xsize,Ysize],1.6/pixscale,0.7/pixscale); Do the interpolation
...
;writefits,outpath+'rss.fits',dataarray
;writefits,outpath+'DataCube.fits',DataCube,head
;writefits,outpath+'Data_wave.fits',wave
;writefits,outpath+'bigmask.fits',bigmask
