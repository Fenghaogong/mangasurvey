; function mlfilterimg

; Smoothes input images by a kernel

; The standard IDL Astronomy library uses a version of
; filter_image that doesn't handle DOUBLE precision arrays,
; instead converting them to FLOAT.  This version properly
; keeps DOUBLE precision if so used for input arrays.
;
; Author:
...
;   v1: Adapted by D.R. Law from standard IDL Astro library routines
; REVISION HISTORY:
;   v2: 02-Oct-2012  D. Law
;       Changed function name to mlfilterimg from mangalib_filterimg to
;       conform with new filename conventions.

function mlfilterimg, image, SMOOTH=width_smooth, ITERATE_SMOOTH=iterate, $
...
;-
  ...
  if N_params() LT 1 then begin
      print,'Syntax - Result = mangalib_filterimg( image, SMOOTH=width, /ALL_PIXELS'
      ...
      return, -1
        ...
        if keyword_set( iterate ) then begin
                ...
                for i=1,nit do  imf = mangalib_filterimg( imf, /SMOOTH, /ALL )
                return,imf
           endif
