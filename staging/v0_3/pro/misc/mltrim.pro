	FUNCTION MLTRIM, NUMBER, FORMAT, FLAG, QUIET=QUIET
;+
; Name        : 
;	TRIM()
; Purpose     : 
...
;
	IF S[0] GT 0 THEN BEGIN
	    ...
	    IF NP GT 1 THEN OUT = MAKE_ARRAY(/STRING,DIMENSION=S[1:S[0]]) $
		    ELSE OUT = 'String'
	    CASE N_PARAMS() OF
		1: FOR I=0,NP-1 DO OUT[I] = MLTRIM(NUMBER[I],QUIET=QUIET)
		2: FOR I=0,NP-1 DO OUT[I] = MLTRIM(NUMBER[I],QUIET=QUIET,FORMAT)
		3: FOR I=0,NP-1 DO OUT[I] = MLTRIM(NUMBER[I],QUIET=QUIET,FORMAT,$
			FLAG)
	    ENDCASE
	    RETURN,OUT
	ENDIF
...
;
	IF TYPE EQ 6 THEN BEGIN
		...
		CASE N_PARAMS() OF
			1:  BEGIN
				RNUMBER = MLTRIM(RNUMBER)
				INUMBER = MLTRIM(INUMBER)
				END
			2:  BEGIN
				RNUMBER = MLTRIM(RNUMBER,FORMAT)
				INUMBER = MLTRIM(INUMBER,FORMAT)
				END
			3:  BEGIN
				RNUMBER = MLTRIM(RNUMBER,FORMAT,FLAG)
				INUMBER = MLTRIM(INUMBER,FORMAT,FLAG)
				END
		ENDCASE
