...
;   v1.6: 4-Jun-2013 D. Law
;       Added /edgesky flag to use edge fibers of blocks as skies
;-

function mlimportspec,wave,redx,obsparam,slitmap,bmap,spec,specivar,xinbundle,yinbundle,fstat,filepath,flavor, doall=doall, edgesky=edgesky
...
if (flavor eq 'full') then begin
  ...
  if nflat gt 1 then flatb1=flatb1[0]
  if nflat eq 0 then  begin
    splog,strcompress('Error: cannot find flatfield.')
    status=-999L
    mlquitmanga3d,status
  endif
  flatr1=file_search(filepath+'spFlat-r1-*',count=nflat)
  if nflat gt 1 then flatr1=flatr1[0]
  if nflat eq 0 then  begin
    splog,strcompress('Error: cannot find flatfield.')
    status=-999L
    mlquitmanga3d,status
  ...
  if ((file_test(Sfileb1))and(file_test(Sfiler1))) then doskysub=0
  ...
  if (doskysub) then begin
    ...
    status=mlskysubtract(fileb1,Sfileb1,obsparam,slitmap,'b1',wave,edgesky=edgesky,plotfile=plotfileb1,flatfile=flatb1)
    status=mlskysubtract(filer1,Sfiler1,obsparam,slitmap,'r1',wave,edgesky=edgesky,plotfile=plotfiler1,flatfile=flatr1)

    ; Use mlskysubtractedge for edge-fiber subtraction
;    if (keyword_set(edgesky)) then begin
      ;status=mlskysubtractedge(fileb1,Sfileb1,obsparam,slitmap,'b1',wave)
      ;status=mlskysubtractedge(filer1,Sfiler1,obsparam,slitmap,'r1',wave)
       ; Kludge for 6653
;      status=mlskysubtractedge6653(fileb1,Sfileb1,obsparam,slitmap,'b1',wave)
;      status=mlskysubtractedge6653(filer1,Sfiler1,obsparam,slitmap,'r1',wave)
;    endif else begin
...
;    endelse
  ...
  if (dofluxcal) then begin
    ...
    if (((size(stdloc))[0] eq 0)or(obsparam[0].plate eq '6655')) then begin
      ...
      ; New modification- use a prior cal file instead
      status=mlfluxcal_prior(Sfileb1,Sfiler1,Cfile1,obsparam,slitmap,wave,plotfile=plotfilec)
     ; If standards exist, do a real calibration
    endif else begin
      status=mlfluxcal(Sfileb1,Sfiler1,Cfile1,obsparam,slitmap,wave,plotfile=plotfilec)
    endelse
